...
function Model(connection, name, schema) {
	...
	var model = function(values) {
		...
		this.remove = function(callback) {
			if(values[primary]) {
				...
				throw new Query.QueryError('Object does not have ' + primary + ' value set.', model);
			}
		};
	};
	...
	model.findOne = function(conditions, projection, options, callback) {
		if(typeof conditions == 'function') {
			callback = conditions;
			options = { };
			projection = null;
		...
		var query = model.find(conditions, projection, options);
		query.limit(1);
		if(typeof callback == 'function') {
