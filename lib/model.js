	...
	Model.prototype.save = function(callback) {
		...
		this.modifiedPaths().forEach(function(path) {
			if(path != primary) {
				if(Array.isArray(schema.paths[path])) {
					...
					this.get(path).forEach(function(document) {
						if(document.id) {
							return document.save();
						} else {
							if(ref) {
								hasMany.push({ ref: ref, document: document });
							} else {
								belongsToMany.push({ Reference: Reference, document: document });
							}
						}
					}, this);
				} else {
		...
		Promise.all(promises)
			...
			.then(function(doc) {
				...
				hasMany.forEach(function(relation) {
					relation.document.set(relation.ref, doc.id);
					promises.push(relation.document.save());
				});
				belongsToMany.forEach(function(relation) {
					promises.push(
						relation.document.save().then(function(document) {
							return new Query.Reference(Class, relation.Reference).create(doc.id, document.id);
						})
					);
				});

				return Promise.all(promises).then(function() { return doc; });
			})
			.then(function(doc) {
				promise.fulfill(doc);
			}, function(err) {
