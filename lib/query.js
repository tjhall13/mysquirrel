...
Query.QueryError = function QueryError(msg, model, fileName, lineNumber) {
	Error.call(this, '{' + model.modelName + '} ' + msg, fileName, lineNumber);
};
util.inherits(Query.QueryError, Error);
		...
		if(param === null) {
			...
			if(typeof param.direction == 'string') {
				if(param.direction.toLowerCase() == 'asc') {
					...
					throw new Query.QueryError('Unknown order: ' + param.direction, { modelName: name });
				}
			} else {
				param.direction = 'DESC';
			}
			modifiers['ORDER BY'] = param.field + ' ' + param.direction;
		}
		return this;
	};
}
