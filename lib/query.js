...
function InsertQuery(model) {
	...
	this._exec = function(connection, callback) {
		connection.query(this._query, this._data, function(err, result) {
		  if(err) {
		    callback(err);
	    } else {
	      model.init(
	        Object.assign({ _id: result.insertId }, model.toObject()),
	        Model.schema.paths
        );
	      callback(null, model);
      }
	...
	this._exec = function(connection, callback) {
		if(this._data) {
	        ...
	        model.init(
		      ...
		      callback(null, model, result.affectedRows);
	      }
		  });
		} else {
			setTimeout(callback, 0, null, model, 0);
		}
	};
...
function ReferenceQuery(Model, Reference) {
	ConditionalQuery.call(this, Model);
	...
	this.create = function(left, right) {
		var table = '__' + ( Model.modelName.localeCompare(Reference.modelName) > 0 ? Reference.modelName + '_' + Model.modelName : Model.modelName + '_' + Reference.modelName ) + '__';

		command = 'INSERT INTO ' + table + ' SET ?';

		this.column(Model.modelName.toLowerCase() + '_id', left, table);
		this.column(Reference.modelName.toLowerCase() + '_id', right, table);

		return this;
	};

	this.delete = function(left, right) {
		var table = '__' + ( Model.modelName.localeCompare(Reference.modelName) > 0 ? Reference.modelName + '_' + Model.modelName : Model.modelName + '_' + Reference.modelName ) + '__';

		command = 'DELETE FROM ' + table;

		this.where(Model.modelName.toLowerCase() + '_id', left, table);
		this.where(Reference.modelName.toLowerCase() + '_id', right, table);

		return this;
	};
}
util.inherits(ReferenceQuery, ConditionalQuery);
